...
orange_xw_yw_bucket = lab_invk(0.28, -0.05, .10, 0)
yellow_xw_yw_bucket = lab_invk(0.28, -0.05, .10, 0)
...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    dest = lab_invk(start_xw_yw_zw[0],start_xw_yw_zw[1],start_xw_yw_zw[2]+0.1, 0)
    move_arm(pub_cmd, loop_rate, dest, vel, accel)
    dest = lab_invk(start_xw_yw_zw[0],start_xw_yw_zw[1],start_xw_yw_zw[2], 0)
    move_arm(pub_cmd, loop_rate, dest, vel, accel)
...
class ImageConverter:
    ...
    def image_callback(self, data):
        ...
        global xw_yw_O # store found green blocks in this list
...
def main():
    ...
    i = 0
    while len(xw_yw_O) > 0:
        xw_yw_O_z = (xw_yw_O[0][0],xw_yw_O[0][1],0.030)
        orange_xw_yw_bucket = lab_invk(-0.1, 0.5, .035*(i+1), 0)
        ret = move_block(pub_command, loop_rate, xw_yw_O_z, orange_xw_yw_bucket, vel, accel)
        if(ret == 0):
            i+=1
    ...
    i = 0
    while len(xw_yw_Y) > 0:
        xw_yw_Y_z = (xw_yw_Y[0][0],xw_yw_Y[0][1],0.030)
        yellow_xw_yw_bucket = lab_invk(-0.1, -0.1, .035*(i+1), 0)
        ret = move_block(pub_command, loop_rate, xw_yw_Y_z, yellow_xw_yw_bucket, vel, accel)
        if(ret == 0):
            i+=1

    # ========================= Student's code ends here ===========================
    gripper(pub_command, loop_rate,0)
